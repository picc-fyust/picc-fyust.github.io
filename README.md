...
物理智能与认知计算研究中心（Research Center for Physical Intelligence and Cognitive Computing，PICC）对外主页。
...
data/            内容（改这里）
  ...
  research.yaml  三个研究方向（含配图文件名）
  people.yaml    成员（教师 / 学生 / 校友）
  publications.yaml  论文
  news.yaml      新闻动态
  join.yaml      加入我们
templates/       页面模板（一般不用改）
assets/          样式、logo、成员照片（assets/img/people/）、方向配图（assets/img/research/）
build.py         生成脚本
tests/           构建自检
dist/            生成结果，可直接上传
...
- 加学生：在 `data/people.yaml` 按注释里的示例增加一条，`tier` 填 `student` / `alumni`；
...
1. 注册 GitHub 账号，新建组织（Organization），例如 `picc-fyust`。
2. 在组织下新建公开仓库，名字必须是 `<组织名>.github.io`，例如 `picc-fyust.github.io`。
