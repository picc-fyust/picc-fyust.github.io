...
def test_home_has_centre_names(dist):
    data = build.load_data(ROOT)
    site = data["site"]
    zh = read(dist, "index.html")
    assert site["name_zh"] in zh
    assert site["name_en"] in zh
    en = read(dist, "en/index.html")
    assert site["name_en"] in en
    assert site["short"] in en
...
def test_faculty_on_people_pages(dist):
    ...
    for person in [p for p in data["people"] if p["tier"] == "faculty"]:
...
def test_faculty_order_matches_data(dist):
    """Faculty appear on the page in the order given in people.yaml."""
    data = build.load_data(ROOT)
    zh = read(dist, "people.html")
    positions = [zh.index(p["name_zh"]) for p in data["people"] if p["tier"] == "faculty"]
...
def test_corresponding_author_marks_without_legend(dist):
    ...
    starred_faculty = [p for p in data["people"] if p["tier"] == "faculty" and any(
        p["name_en"] + "*" in q["authors"] for q in starred)]
    assert starred_faculty, "no faculty is marked as corresponding author"
    for person in starred_faculty:
        assert f"<strong>{person['name_en']}</strong>*" in html, person["name_en"]
...
def test_only_members_are_bolded(dist):
    """Member names are highlighted in author lists; outside collaborators are not."""
    data = build.load_data(ROOT)
    html = read(dist, "publications.html")
    members = {p["name_en"] for p in data["people"]}
    for name in re.findall(r"<strong>([^<]+)</strong>", html):
        base = name.rstrip("*").strip()
        assert base in members or re.fullmatch(r"[A-Z]\.( [A-Z]\.)* [A-Za-z-]+", base), base
    assert any(f"<strong>{p['name_en']}</strong>" in html
               for p in data["people"] if p["tier"] == "faculty")
